...
app.post("/send_message/:username", async (req, res) => {
  ...
  try {
    ...
    res.json({
      status: "success",
      data: { from: result.srcId, to: result.dstId, content: result.content },
    });
  } catch (e) {
    if (e.status === "fail") {
      res.status(400).json({ status: e.status, data: e.dataError });
...
app.get("/read_message/", async (req, res) => {
  // A implementer
  try {
    const result = await db.readMessage(req.userId);
